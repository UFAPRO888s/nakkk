...
const configValues = {
	...
	'host.name': {
		...
		validate: (config) => {
			...
			if (names.length !== 0) {
				return 'อนุญาตเฉพาะชื่อโดเมนเท่านั้น';
			}
...
export default function Settings(props) {
	...
	const handleChange = (what) => (event) => {
		...
		if (!(what in configValues)) {
			console.warn(`ค่า config "${what}" ไม่ใช่ configValue ที่ถูกต้อง`);
			return;
		}
	...
	const handleSave = async () => {
		...
		if (err !== null) {
			if (err.code === 404) {
				notify.Dispatch('error', 'บันทึก:ตั้งค่า', i18n._(t`ไม่พบปลายทาง API ไม่ได้บันทึกการตั้งค่า`));
			} else if (err.code === 409) {
				notify.Dispatch('error', 'บันทึก:ตั้งค่า', i18n._(t`มีข้อผิดพลาดบางอย่างในการตั้งค่า ไม่ได้บันทึกการตั้งค่า`));
			} else {
				notify.Dispatch('error', 'บันทึก:ตั้งค่า', i18n._(t`เกิดปัญหาในการจัดเก็บการตั้งค่า ไม่ได้บันทึกการตั้งค่า`));
			}
			...
			notify.Dispatch('success', 'บันทึก:ตั้งค่า', i18n._(t`การตั้งค่าที่บันทึกไว้. การเปลี่ยนแปลงทั้งหมดจะถูกนำไปใช้หลังจากรีสตาร์ทแอปพลิเคชัน`));
	...
	if ($config.data === null) {
		return (
			<Paper xs={8} sm={6} md={6}>
				...
				<PaperContent>
					<Trans>ไม่สามารถโหลดการกำหนดค่า.</Trans>
				</PaperContent>
				<PaperFooter
					buttonsRight={
		...
		<React.Fragment>
					...
					<TabsVerticalGrid>
							...
							<Grid container spacing={2}>
								...
								<Grid item xs={12}>
									<Typography variant="body1">
										<Trans>การตั้งค่าระบบที่สำคัญทั้งหมด.</Trans>
									</Typography>
								</Grid>
								{$tabs.general.errors && (
									<Grid item xs={12}>
										...
										<BoxText color="success">
											<Typography variant="inherit" color="inherit" width="100%">
												<Link
													...
													target="_blank"
												>
													<Trans>มีการปรับปรุงที่มีอยู่ ที่นี่คุณจะได้รับข้อมูลเพิ่มเติม.</Trans>
												</Link>
											</Typography>
										</BoxText>
									)}
						...
						<TabPanel value={$tab} index="service" className="panel">
							<Grid container spacing={2}>
								...
								<Grid item xs={12}>
									<Typography variant="body1">
										<Trans>การตั้งค่าการเชื่อมต่อกับบริการ.</Trans>{' '}
										<Link color="secondary" href="#" target="_blank">
											<Trans>เพิ่มเติมเกี่ยวกับบริการ</Trans>
										</Link>
										.
									</Typography>
								</Grid>
								<Grid item xs={12}>
									<Checkbox
										label={<Trans>บริการรีสตรีมเมอร์</Trans>}
								...
								<Grid item xs={12}>
									...
									<Typography variant="caption">
										<Trans>ตัวตนที่ไม่ซ้ำใครในบริการ.</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
									<TextField
								...
								<Grid item xs={12}>
									...
									<Typography variant="caption">
										<Trans>โทเค็นบริการสำหรับการตรวจสอบ</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
									<TextField
						...
						<TabPanel value={$tab} index="network" className="panel">
							<Grid container spacing={2}>
								...
								<Grid item xs={12}>
									<TextField
										label={<Trans>สาธารณสมบัติ/s</Trans>}
										env={env('host.name')}
								...
								<Grid item xs={12}>
									...
									<Typography variant="caption">
										<Trans>
											โปรดป้อนที่อยู่อีเมลของคุณเพื่อแสดงข้อตกลงกับข้อกำหนดในการให้บริการของ Let's Encrypt CA
											และเพื่อรับการแจ้งเตือนในกรณีที่มีปัญหา
										</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
								...
								<Grid item xs={12} md={12}>
									...
									<Typography variant="caption">
										<Trans>
											กำหนดขีดจำกัดการดูสำหรับเซสชัน HLS หากเกินขีดจำกัด ผู้ดู HLS จะได้รับรหัสสถานะ HTTP 509 (แบนด์วิธเกินขีดจำกัด) 0
											ไม่จำกัด
										</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12} md={12}>
									...
									<Typography variant="caption">
										<Trans>
											ตั้งค่าขีดจำกัดแบนด์วิธเป็น Mbit ต่อวินาทีสำหรับการถ่ายโอนข้อมูล HLS ขาออก บริการทั้งหมด เช่น RTMP และขาออก
											กระบวนการรวมอยู่ในการคำนวณ หากแบนด์วิธเกิน ผู้ดู HLS จะได้รับรหัสสถานะ HTTP 509 (เกินขีดจำกัดแบนด์วิธ) 0 ไม่จำกัด
										</Trans>
									</Typography>
								</Grid>
							</Grid>
						...
						<TabPanel value={$tab} index="auth" className="panel">
							<Grid container spacing={2}>
								...
								<Grid item xs={12}>
									...
									<Typography variant="caption">
										<Trans>ขอแนะนำให้เปิดใช้งานการให้สิทธิ์ มิฉะนั้น ใครๆ ก็สามารถเข้าถึงอินสแตนซ์นี้ได้</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
									<TextField
						...
						<TabPanel value={$tab} index="playback" className="panel">
							<Grid container spacing={2}>
								...
								<Grid item xs={12}>
									...
									<Typography variant="caption">
										<Trans>
											รายการช่วง IP ในรูปแบบ CIDR เช่น 127.0.0.1/32 ที่สถิติจะไม่บันทึก—หนึ่งช่วง IP ต่อบรรทัด
											เว้นว่างไว้เพื่อบันทึกเซสชันทั้งหมด
										</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
									...
									<Typography variant="caption">
										<Trans>เวลาจนกว่าการเชื่อมต่อของผู้ดูที่ไม่ได้ใช้งานจะถือว่าปิด.</Trans>
									</Typography>
								</Grid>
								<Grid item xs={12}>
									<Checkbox
			...
			<Backdrop open={$restart.restarting}>
				<Paper xs={4} sm={4} md={4}>
					<PaperHeader title={<Trans>รีสตาร์ท</Trans>} />
					<PaperContent>
						{$restart.timeout === false ? (
							...
							<React.Fragment>
								...
								<Typography variant="body1" sx={{ mt: '1em' }}>
									<Trans>
										หากคุณเปิดใช้งาน Let's Encrypt TLS อาจใช้เวลาสักครู่เพื่อรับใบรับรอง ตรวจสอบให้แน่ใจว่า Restreamer Core เป็น
										สามารถเข้าถึงได้ผ่านพอร์ต 80 จากอินเทอร์เน็ต โปรดตรวจสอบบันทึกคอนโซลของ Restreamer Core
									</Trans>
								</Typography>
								<Typography variant="body1" sx={{ mt: '1em' }}>
									<Trans>
										หากคุณเปลี่ยนพอร์ต อาจเป็นไปได้ว่า Restreamer Core เริ่มการทำงานใหม่แล้ว แต่ตอนนี้มีให้ใช้งานบนพอร์ตอื่นแล้วท่าเรือ.
									</Trans>
								</Typography>
							</React.Fragment>
						)}
			...
			>
				<Typography variant="body1">
					<Trans>คุณเปลี่ยนการกำหนดค่าแล้ว เพื่อให้การเปลี่ยนแปลงมีผล คุณต้องรีสตาร์ทแอปพลิเคชัน คุณต้องการรีสตาร์ทเดี๋ยวนี้หรือไม่?</Trans>
				</Typography>
			</Dialog>
			<Dialog
			...
			>
				<Typography variant="body1">
					<Trans>คุณต้องการรีสตาร์ทแอปพลิเคชันทันทีหรือไม่</Trans>
				</Typography>
			</Dialog>
		</React.Fragment>
	);
